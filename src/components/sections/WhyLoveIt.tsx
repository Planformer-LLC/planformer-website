...
export default function WhyLoveIt() {
  return (
    <section className="bg-white pt-16 pb-0 md:py-16">
      ...
      <div className="grid w-full gap-10 md:grid-cols-[minmax(0,680px)_minmax(0,1fr)] md:items-start md:gap-12">
        {/* LEFT IMAGE: crop from bottom */}
        <Reveal>
          <div
            className="
              overflow-hidden bg-white shadow-sm
              md:rounded-tr-[20px] md:rounded-br-[20px]
              md:h-[730px] md:w-[720px]
            "
          >
            <Image
        ...
        <div className="w-full px-4 md:pl-12 md:pr-16 md:pt-5">
          <Reveal>
          ...
          <div className="mt-7 space-y-4">
            {items.map((i, idx) => (
              <Reveal key={i.title} delay={idx * 0.06}>
                {/* ✅ Mobile same. Desktop wider: md:max-w-[460px] -> md:max-w-[560px] */}
                <div className="w-full rounded-2xl bg-[#F5F5F5] md:max-w-[560px]">
          ...
          <Reveal>
            {/* Mobile same. Desktop same behavior. Icon replaced with Lucide */}
            <Link
              href="/download"
